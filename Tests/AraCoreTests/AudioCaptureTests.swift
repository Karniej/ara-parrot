...
struct AudioCaptureTests {
    ...
    /// callback exits.
    ///
    /// ## What changed when `retire` arrived
    ///
    /// This test used to end by asserting the engine was released within a
    /// second of the callback returning. That is now false on purpose.
    /// `retire` holds a replaced backend for `retirementHold` before letting
    /// it go, because prompt release is what the AudioUnit property listener's
    /// queued blocks crash on — see `AudioCapture.retire`.
    ///
    /// The two fixes point the same way and the assertion is the only part
    /// that conflicted: one keeps the engine alive across the *notification
    /// callback*, the other keeps it alive across *anything AVFAudio still has
    /// queued*. What is checked here is the first, which is the part this seam
    /// can actually observe; the release itself is deferred, and asserting a
    /// ten-second wait would buy nothing but a slow test.
    ...
    func notificationSourceOutlivesCallback() throws {
        ...
        // Still alive, and that is the contract now: `retire` holds it rather
        // than releasing it under whatever AVFAudio has queued. The callback
        // assertions above are what prove it outlived the notification.
        #expect(firstReleased.wait(timeout: .now()) == .timedOut)
        #expect(first.value != nil)
